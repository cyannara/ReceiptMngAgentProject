...
def save_db_node(state: ReceiptAgentState):
    # 실행 위치에 따라 프로젝트 루트와 backend 디렉터리를 모두 경로에 추가
    import sys
    ...
    try:
        from backend.db.save_local_db import save_local_db, DEFAULT_DB_TARGET
    except Exception as e:
        try:
            from db.save_local_db import save_local_db, DEFAULT_DB_TARGET
