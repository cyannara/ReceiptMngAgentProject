...
with st.sidebar:
    ...
    st.session_state["ocr_provider"] = st.selectbox(
        "OCR 엔진",
        options=["gpt", "upstage"],
        index=["gpt", "upstage"].index(st.session_state.get("ocr_provider", "gpt")),
        help="GPT Vision 또는 Upstage Document Parser 중 선택하세요.",
    )
...
with col1:
    ...
    if uploaded_file is not None:
        ...
        st.image(image, use_container_width=True, caption="업로드된 영수증 이미지")
        ...
        if not user_id.strip():
            ...
            with st.spinner("백엔드 에이전트 파이프라인 작동 중 (OCR 및 RAG 감사 연동)..."):
                try:
                    ...
                    # 수정: 쿼리 스트링 매개변수로 user_id 및 OCR provider 전달
                    params = {
                        "user_id": user_id.strip(),
                        "ocr_provider": st.session_state.get("ocr_provider", "gpt"),
                    }
